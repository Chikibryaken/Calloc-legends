...
void fibonacciSort(long arr[], int n) {
    memory = 0;
    cntMain = 0;
    int fib1 = 0, fib2 = 1, fib3 = 1, i, j, temp;
    cntAux = 3;
    while (fib3 < n) {
        fib1 = fib2;
        fib2 = fib3;
    ...
    while (fib1 > 0) {
        for (cntAux++, i = fib3; i < n; cntAux++, i++) {
            for (cntAux++, j = i; j >= fib3 && arr[j - fib3] > arr[j]; cntAux++, j -= fib3) {
                temp = arr[j];
                arr[j] = arr[j - fib3];
                arr[j - fib3] = temp;
                cntMain += 3
